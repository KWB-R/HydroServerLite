...
<table width="960" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <input id="uploadfiles" name="upfile" type="button" value="<?php echo $Upload; ?>">
        ...
        <tr>
          ...
          <input name="Button" type="button" class="button" id="submit1" value="<?php echo $SubmitData; ?>" style="width: 135px"/>
            <input type="button" style=" float:right" value="Test" id='compare' />
 ...
 <div style="overflow: hidden;" id="windowContent">
  ...
  <!-- Some Hidden HTML items that help transfer data between the Javascript and the jQuery regions of the page. -->
  <div id ="filenameHidden" hidden="true"></div>
  <div id ="idHidden" hidden="true"></div>
  <button id="hiddenTriger" type="button" hidden="true">Click Me!</button>
   <button id="hiddenTriger2" type="button" hidden="true" onClick="checkMain()">Click Me!</button>
	<p>&nbsp;</p></blockquote>
    </td>
  </tr>
  <tr>
...
//New Check function that first gets the datetime type from the user. 



//Below one is the old one.
function check(name, id2)
{

//Trigger the Jquery function from the hidden button
document.getElementById('filenameHidden').innerHTML = name;
document.getElementById('idHidden').innerHTML = id2;
document.getElementById("hiddenTriger").click();
}

function checkMain()
{

name = 	document.getElementById('filenameHidden').innerHTML;
id2 = document.getElementById('idHidden').innerHTML;
...
//Adding dateformat Describer

$("#compare").jqxButton({ width: '250', height: '25', theme: 'darkblue'});
$('#window').jqxWindow({ maxHeight: 800, maxWidth: 800, minHeight: 200, minWidth: 200, height: 340, width: 500, theme: 'darkblue' });
$('#window').jqxWindow('hide');
$("#hiddenTriger").click(function(){
$("html, body").animate({ scrollTop: 0 }, "slow");
$('#window').jqxWindow('show');
var fileName = $('#filenameHidden').text();

$('#windowContent').load('formatDate.php?name='+fileName, function() {
		
		$("#submitDateF").click(function(){
			
			//DateValues are entered. Time To process the form. 
			
			$.post( "dateFormatter.php", $( "#dateFormat" ).serialize() )
			.done(function(data) {
   			 	if (data == "Done Converting")
				{
					alert("Date formatting done.");
					$('#window').jqxWindow('hide');
					$("#hiddenTriger2").click();
				}
				else
				{
					alert("Processing Error. Please Try uploading again. Error: "+data);
					$('#window').jqxWindow('hide');
					location.reload();
				}
 			 });
...
$("#submit1").click(function() {
	...
	/**
	Disabling the checks as this is multiple data import form now!!
...
$.ajax({
  type: "POST",
  url: "do_import_data_file.php?SourceID="+sourceid+"&SiteID="+siteid+"&filename="+$filename+"&special=1"
}).done(function( msg ) {
  if(msg==1)
  {
	   $("#statusmsg").show(1200);
