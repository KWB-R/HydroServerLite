...
class Wfs extends CI_Controller
{
	...
	function write_xml()
	{
			...
			if( $this->input->get('request') == NULL && $this->input->post('request') == NULL && $this->input->get('REQUEST') == NULL && $this->input->post('REQUEST') == NULL)
		{
			//No request found
			header('Content-Type:text/xml; charset=UTF-8', TRUE);
			$this->load->view('services/request_error');
			return;
		}
		else
		{
		...
		else
		{
			...
			if($queryID != $definedQuery):
				$this->load->view('services/get_feature_error'); return;
			endif;
		...
		case 'GetFeature':
		  ...
		  if (count($parts)>1)
		  {
			...
			switch($version):
					case '1.0.0' : $this->load->view('services/get_feature', $data); break;
					case '2.0.0' : $this->load->view('services/get_feature_v2', $data); break;
					default : //Defaulting to version 2.0.0
						 $this->load->view('services/get_feature_v2', $data); break;
				endswitch;
		...
		case 'DescribeFeatureType':
			...
			switch($version):
				case '1.0.0' : $this->load->view('services/describe_feature_type'); break;
				case '2.0.0' : $this->load->view('services/describe_feature_type_v2'); break;
				default : //Defaulting to version 2.0.0
					 $this->load->view('services/describe_feature_type_v2'); break;
			endswitch;
		...
		case 'GetCapabilities':
			...
			switch($version):
				case '1.0.0' : $this->load->view('services/get_capabilities', $data); break;
				case '2.0.0' : $this->load->view('services/get_capabilities_v2', $data); break;
				default : //Defaulting to version 2.0.0
					 $this->load->view('services/get_capabilities_v2', $data); break;
			endswitch;
		...
		case 'DescribeStoredQueries':
			...
			$this->load->view('services/dec_stored_queries'); 
		...
		case 'ListStoredQueries':
			...
			header('Content-Type:text/xml; charset=UTF-8', TRUE);
			//This feature only exists in version 2.0.0, so no support for version 1.0.0
			$this->load->view('services/list_stored_queries'); 
		break;
	...
	function variables()
	{
		...
		//print_r($variables);
		$this->load->view('services/variables', $data);
	}
