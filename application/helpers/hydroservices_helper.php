...
if (!function_exists('RunService')) {
    ...
    function RunService() {
		...
		if ($action_is_set == 1) {
			...
			$name_file = APPPATH.'views/services/operations/operation_' . $operation_name . '.html';
			$name = @implode ('', @file (APPPATH.'views/services/operations/operation_' . $operation_name . '.html'));
			...
			// Return the WSDL
			$wsdl = @implode ('', @file (APPPATH.'views/services/wateroneflow.wsdl'));
			if (strlen($wsdl) > 1) {
  			...
  			header("Content-Type: text/html");
  			$asmx_file_name = APPPATH.'views/services/asmx_page.html';
...
if (!function_exists('generateObsMetadata')) {
	...
	function generateObsMetadata($sourceID=NULL,$sampleMedium = "Unknown") {
		...
		$abstractMetadata="";
if(array_key_exists('Abstract',$metadataInfo))
{$abstractMetadata=$metadataInfo['Abstract'];
}
