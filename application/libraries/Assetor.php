...
class Assetor {
	...
	function __construct()
	{
		...
		$temp = $this->_CI->config->item('less_folder', 'assetor');
		$this->_less_folder = empty($temp) ? $this->_css_folder : $this->_CI->config->item('less_folder', 'assetor');
		$this->_lessphp_folder = $this->_CI->config->item('lessphp_folder','assetor');
		$temp = $this->_CI->config->item('css_min_folder', 'assetor');
		$this->_css_min_folder = empty($temp) ? $this->_css_folder : $this->_CI->config->item('css_min_folder', 'assetor');
		$this->_js_folder = $this->_CI->config->item('js_folder', 'assetor');
		$temp = $this->_CI->config->item('js_min_folder', 'assetor');
		$this->_js_min_folder = empty($temp) ? $this->_js_folder : $this->_CI->config->item('js_min_folder', 'assetor');
	...
	*/
	...
	{
		$assets = '';
		if(file_exists($this->_css_min_folder.$group.'.css') || file_exists($this->_js_min_folder.$group.'.js'))
		{
				if(file_exists($this->_css_min_folder.$group.'.css'))
			{
			...
			if(file_exists($this->_js_min_folder.$group.'.js'))
			{
		...
		else
		{
			...
			foreach($this->_styles[$group] as $filetype=>$file)
			{
					switch ($file['ext']) {
						case 'css':
